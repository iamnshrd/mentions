"""Canonical workflow entrypoint for retrieval."""
...
from agents.mentions.storage.persistence_helpers import persist_analysis_stub, persist_ticker_news
from agents.mentions.workflows.context_retrieval import (
    retrieve_market_context,
    retrieve_news_context,
    retrieve_transcript_context,
)
from agents.mentions.workflows.ticker_retrieval import (
...
from agents.mentions.services.knowledge import query_pmt_knowledge_bundle
...
def retrieve_transcripts(frame: dict) -> dict:
...
def retrieve_news(frame: dict) -> dict:
    ...
    try:
        ...
        bundle = build_news_context_bundle(
            query,
            category=category,
            market_data=frame.get('market_data'),
            limit=int(frame.get('limit', 5) or 5),
            require_live=False,
        )
        if not bundle.get('news'):
            stored = search_news_runtime(query=query, limit=int(frame.get('limit', 5) or 5))
