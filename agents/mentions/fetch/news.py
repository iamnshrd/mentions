...
from agents.mentions.fetch.gdelt import GdeltProviderUnavailable, fetch_gdelt_news
...
def fetch_news_with_status(query: str, category: str = 'general',
    ...
    try:
        live = _fetch_live_news(query, category=category, limit=limit, topic_hints=topic_hints, speaker_hint=speaker_hint)
        if live:
