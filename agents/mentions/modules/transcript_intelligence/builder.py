...
from agents.mentions.module_contracts import ensure_list, normalize_status
...
def build_transcript_intelligence_bundle(query: str, limit: int = 5, speaker: str = '') -> dict:
    ...
    try:
        from agents.mentions.modules.transcript_knowledge_extraction import extract_transcript_knowledge_bundle
...
def _search_transcript_candidates(effective_query: str, entities: dict, speaker: str, limit: int = 5) -> list[dict]:
    ...
    if format_hints:
        format_only_rows = search_transcript_tags_runtime(
            speaker=speaker,
            topic_tags=[],
            format_tags=format_hints,
            limit=limit * 6,
    ...
    runtime_rows = []
    if speaker:
        runtime_rows = ensure_list(search_transcripts_runtime(query=speaker, speaker=speaker, limit=limit * 10))
    explicit_rows = []
    if speaker and format_hints:
        for fmt in format_hints:
            explicit_rows.extend(ensure_list(search_transcripts_runtime(speaker=speaker, title_query=fmt, limit=limit * 10)))
    for hint in event_hints:
        if speaker:
            explicit_rows.extend(ensure_list(search_transcripts_runtime(speaker=speaker, title_query=hint, limit=limit * 10)))
    runtime_by_event = {}
