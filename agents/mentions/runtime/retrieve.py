...
from agents.mentions.utils import timed, get_threshold, fts_query
...
def retrieve_market_data(frame: dict) -> dict:
    ...
    if ticker:
        try:
            from agents.mentions.fetch.kalshi import get_market, get_history
        ...
        try:
            from agents.mentions.fetch.kalshi import search_markets
    ...
    try:
        from agents.mentions.kb.query import query_analysis_cache
...
def retrieve_transcripts(frame: dict) -> list[dict]:
    ...
    try:
        ...
        chunks = search_transcripts(query, limit=limit)
...
@timed('retrieve_news')
def retrieve_news(frame: dict) -> tuple[list[dict], str]:
    """Retrieve recent news context for the market category."""
    category = frame.get('category', 'general')
    query = frame.get('query', '')

    try:
        from agents.mentions.capabilities.news_context.api import fetch_news
        return fetch_news(
            query,
            category=category,
...
def build_retrieval_bundle(query: str, frame: dict) -> dict:
    ...
    news, news_status = retrieve_news(frame)
