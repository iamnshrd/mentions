...
def _add_column_if_missing(conn, table: str, column: str, ddl_type: str) -> None:
    ...
    if not _has_table(conn, table):
        log.debug('Skipping column add for missing table %s.%s', table, column)
        return
...
def _v3(conn) -> None:
    ...
    via :mod:`agents.mentions.storage.retrieval.embed_cache`, which also owns the
...
def _v6(conn) -> None:
    ...
    if _has_table(conn, 'decision_cases'):
        cur = conn.cursor()
        cur.executescript('''
...
def _v7(conn) -> None:
    ...
    * ``heuristic_applications.regime`` TEXT — free-form regime tag
      ...
      the heuristic-learning analysis layer.
    ...
    * ``decision_cases.outcome`` INTEGER — binary 0/1 win marker
    ...
    if _has_table(conn, 'heuristic_applications'):
        conn.execute(
            'CREATE INDEX IF NOT EXISTS idx_heuristic_apps_regime '
            'ON heuristic_applications(regime)'
        )
    if _has_table(conn, 'speaker_stance_applications'):
        conn.execute(
...
def _v9(conn) -> None:
    ...
    JSON array (see ``agents.mentions.services.speakers.canonicalize``).
    ...
    if _has_table(conn, 'transcript_chunks'):
        conn.execute(
