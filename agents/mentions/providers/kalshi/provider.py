...
from . import client as kalshi_client
...
def get_event_bundle(event_ticker: str, with_nested_markets: bool = False) -> dict:
    payload = kalshi_client.get_event(event_ticker, with_nested_markets=with_nested_markets)
...
def get_history_bundle(ticker: str, series_ticker: str = '', days: int = 30) -> dict:
    history = kalshi_client.get_history(ticker, series_ticker=series_ticker, days=days)
...
def search_markets_bundle(query: str, limit: int = 10) -> dict:
    ...
    markets = kalshi_client.search_markets(query, limit=limit)
...
def get_markets_bundle(category: str = '', limit: int = 20, status: str = 'open', event_ticker: str = '') -> dict:
    ...
    markets = kalshi_client.get_markets(category=category, limit=limit, status=status, event_ticker=event_ticker)
...
def get_top_movers_bundle(limit: int = 10) -> dict:
    markets = kalshi_client.get_top_movers(limit=limit)
