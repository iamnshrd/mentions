...
from mentions_domain.normalize import ensure_dict, ensure_list, normalize_status
from mentions_domain.market_resolution import extract_market_entities
from agents.mentions.providers.news import GoogleNewsRssUnavailable, fetch_google_news_rss
...
def build_news_context_bundle(query: str, category: str = 'general',
    ...
    news = _merge_news_items([], google_news_items)
    ...
    raw_news = list(news)
    trace_log('news.builder.sources', run_id=run_id, google_news_count=len(google_news_items), rss_count=0, merged_count=len(news), status=status)
    ...
    try:
        ...
        if kept_items:
            ...
            news = raw_news
            if raw_news and status == 'unavailable':
                status = 'live'
            elif not raw_news:
                status = 'unavailable'
    ...
    separated_news = _separate_news_views(news=news, typed_news=typed_news)
    ...
    context_risks = _build_context_risks(
        status=status,
        freshness=freshness,
    ...
    ranking_debug = _build_ranking_debug(
        relevance=relevance,
        rss_bundle=rss_bundle,
        google_news_bundle=google_news_bundle,
        status=status,
        freshness=freshness,
        sufficiency=sufficiency,
        news=news,
    ...
    return {
        ...
        'summary_sections': _build_summary_sections(separated_news),
        ...
        'typed_news': typed_news,
        'direct_event_news': separated_news.get('direct_event_news', []),
        'background_news': separated_news.get('background_news', []),
...
def _build_ranking_debug(*, relevance: dict, rss_bundle: dict, google_news_bundle: dict,
                         status: str, freshness: str, sufficiency: str,
                         news: list[dict], typed_news: dict,
                         context_risks: list[str], quality_signals: dict) -> dict:
    relevance = ensure_dict(relevance)
    rss_bundle = ensure_dict(rss_bundle)
    google_news_bundle = ensure_dict(google_news_bundle)
    typed_news = ensure_dict(typed_news)
    ranked_items = ensure_list(relevance.get('ranked_items', []))
    kept_items = ensure_list(relevance.get('kept_items', []))
    rejected_items = ensure_list(relevance.get('rejected_items', []))
    return {
        'status': status,
        'freshness': freshness,
        'sufficiency': sufficiency,
        'provider_coverage': {
            'rss_status': rss_bundle.get('provider_status', rss_bundle.get('status', '')),
            'rss_count': len(ensure_list(rss_bundle.get('raw_items', []))),
            'google_news_status': google_news_bundle.get('provider_status', google_news_bundle.get('status', '')),
            'google_news_count': len(ensure_list(google_news_bundle.get('raw_items', []))),
        },
        'ranking_summary': {
            'ranked_count': len(ranked_items),
            'kept_count': len(kept_items),
            'rejected_count': len(rejected_items),
            'final_news_count': len(ensure_list(news)),
        },
        'typed_coverage': {
            'coverage_state': typed_news.get('coverage_state', 'empty'),
            'core_count': len(ensure_list(typed_news.get('core_news', []))),
            'expansion_count': len(ensure_list(typed_news.get('expansion_news', []))),
            'ambient_count': len(ensure_list(typed_news.get('ambient_news', []))),
        },
        'context_risks': ensure_list(context_risks),
        'quality_signals': ensure_dict(quality_signals),
        'lead_news': _compact_news_item(ensure_dict(news[0])) if news else {},
        'top_ranked': [_compact_ranked_item(row) for row in ranked_items[:3]],
        'top_rejected': [_compact_ranked_item(row) for row in rejected_items[:3]],
    }


def _build_context_risks(*, status: str, freshness: str, sufficiency: str,
                         news: list[dict], relevance: dict, typed_news: dict) -> list[str]:
    relevance = ensure_dict(relevance)
    typed_news = ensure_dict(typed_news)
    risks = []
    kept_items = ensure_list(relevance.get('kept_items', []))
    ...
    ranked_items = ensure_list(relevance.get('ranked_items', []))
    noise_flags = []
    for row in rejected_items[:5]:
        noise_flags.extend(ensure_list(ensure_dict(row).get('noise_flags', [])))

    if not news:
        risks.append('no-news-context')
    if status == 'partial':
        risks.append('partial-provider-coverage')
    if status == 'cache':
        risks.append('cache-only-news')
    if freshness == 'missing':
        risks.append('missing-fresh-news')
    elif freshness == 'stale':
        risks.append('stale-news-context')
    if sufficiency == 'weak':
        risks.append('weak-news-context')
    elif sufficiency == 'limited':
        risks.append('limited-news-context')

    coverage_state = typed_news.get('coverage_state', 'empty')
    if coverage_state == 'ambient-only':
        risks.append('ambient-only-news')
    elif coverage_state == 'topic-led':
        risks.append('indirect-event-news')
    elif coverage_state == 'empty' and news:
        risks.append('untyped-news-context')

    if ranked_items and not kept_items:
        risks.append('relevance-rejected-all')
    if 'generic-context-only' in noise_flags:
        risks.append('generic-news-context')
    if 'low-quality-source' in noise_flags:
        risks.append('low-quality-news-sources')
    if 'strict-event-anchor-missing' in noise_flags:
        risks.append('missing-event-anchor')
    if 'speaker-not-mentioned' in noise_flags:
        risks.append('speaker-not-explicit-in-news')
    if 'event-frame-not-mentioned' in noise_flags:
        risks.append('event-frame-not-explicit-in-news')
...
def _build_quality_signals(*, status: str, freshness: str, sufficiency: str,
                           relevance: dict, typed_news: dict) -> dict:
    relevance = ensure_dict(relevance)
    typed_news = ensure_dict(typed_news)
    kept_items = ensure_list(relevance.get('kept_items', []))
    ranked_items = ensure_list(relevance.get('ranked_items', []))
    lead = ensure_dict(kept_items[0] if kept_items else ranked_items[0] if ranked_items else {})
    return {
        'provider_status': status,
        'freshness': freshness,
        'sufficiency': sufficiency,
        'coverage_state': typed_news.get('coverage_state', 'empty'),
        'has_ranked_hits': bool(ranked_items),
        'has_kept_hits': bool(kept_items),
        'lead_score': lead.get('final_relevance_score', 0) if lead else 0,
        'lead_decision': lead.get('decision', '') if lead else '',
    }
...
def _compact_ranked_item(item: dict) -> dict:
    item = ensure_dict(item)
    return {
        'headline': item.get('headline', '') or item.get('title', ''),
        'source': item.get('source', ''),
        'decision': item.get('decision', ''),
        'final_relevance_score': item.get('final_relevance_score', 0),
        'topic_matches': ensure_list(item.get('topic_matches', [])),
        'event_hint_matches': ensure_list(item.get('event_hint_matches', [])),
        'event_anchor_matches': ensure_list(item.get('event_anchor_matches', [])),
        'event_phrase_matches': ensure_list(item.get('event_phrase_matches', [])),
        'noise_flags': ensure_list(item.get('noise_flags', [])),
    }


def _separate_news_views(*, news: list[dict], typed_news: dict) -> dict:
    typed_news = ensure_dict(typed_news)
    core_news = ensure_list(typed_news.get('core_news', []))
    expansion_news = ensure_list(typed_news.get('expansion_news', []))
    ambient_news = ensure_list(typed_news.get('ambient_news', []))
    direct_event_news = core_news or expansion_news
    background_news = ambient_news

    if not direct_event_news and news and not background_news:
        direct_event_news = ensure_list(news[:3])
    elif not background_news and news:
        used = {
            (ensure_dict(item).get('headline', ''), ensure_dict(item).get('url', ''))
            for item in direct_event_news
        }
        background_news = [
            item for item in ensure_list(news)
            if (ensure_dict(item).get('headline', ''), ensure_dict(item).get('url', '')) not in used
        ][:3]

    return {
        'direct_event_news': direct_event_news[:3],
        'background_news': background_news[:3],
    }


def _build_summary_sections(separated_news: dict) -> dict:
    separated_news = ensure_dict(separated_news)
    direct_event_news = ensure_list(separated_news.get('direct_event_news', []))
    background_news = ensure_list(separated_news.get('background_news', []))
    direct_summary = '; '.join(
        [
            ensure_dict(item).get('headline', '') or ensure_dict(item).get('title', '')
            for item in direct_event_news[:3]
            if ensure_dict(item).get('headline') or ensure_dict(item).get('title')
        ]
    )
    background_summary = '; '.join(
        [
            ensure_dict(item).get('headline', '') or ensure_dict(item).get('title', '')
            for item in background_news[:3]
            if ensure_dict(item).get('headline') or ensure_dict(item).get('title')
        ]
    )
    return {
        ...
        'lead_direct_news': _compact_news_item(ensure_dict(direct_event_news[0])) if direct_event_news else {},
        'lead_background_news': _compact_news_item(ensure_dict(background_news[0])) if background_news else {},
    }
...
def _typed_news_rows(*, query: str, event_title: str, news: list[dict], run_id: str, families: list[str]) -> tuple[dict, list[dict]]:
    from agents.mentions.services.transcripts.semantic_retrieval.client import news_score
