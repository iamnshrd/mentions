...
from agents.mentions.services.transcripts.semantic_retrieval.client import family_score as remote_family_score
from agents.mentions.services.transcripts.semantic_retrieval.family_taxonomy import TRANSCRIPT_FAMILY_TAXONOMY_V0
from agents.mentions.services.transcripts.semantic_retrieval.strategy import retrieve_family_segments
...
def _candidate_trace(meta: dict, *, tid: int, title: str) -> dict:
    metadata = meta.get('metadata') or {}
    return {
        'transcript_id': meta.get('transcript_id') or tid,
        'segment_index': meta.get('segment_index'),
        'source': meta.get('source', ''),
        'source_ref': meta.get('source_ref', ''),
        'event_title': meta.get('event_title') or title,
...
def _candidate_hit(*, tid: int, title: str, speaker: str, family: str, quote: str, adjusted_score: float, evidence_type: str, meta: dict | None = None) -> dict:
    ...
    return {
        ...
        'trace': _candidate_trace(meta, tid=tid, title=title),
    }
