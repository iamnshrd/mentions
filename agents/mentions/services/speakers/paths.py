...
from agents.mentions.services.events.prior_registry import detect_event_prior_mode
from agents.mentions.services.media.detection import analyze_media_appearance
...
def _event_support_profile(event_ctx: dict, transcript_bundle: dict, evidence_view: dict) -> dict:
    ...
    if not (core_hits or spillover_hits or generic_regime_hits):
        ...
        title = event_ctx.get('event_title') or event_ctx.get('title') or ''
        event_prior_mode = detect_event_prior_mode(title, fmt)
