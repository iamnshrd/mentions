...
- `mentions_core/` + `agents/mentions/` coexist, and some docs still reflect pre-cutover history
...
Notes:
- `runtime/`, `analysis/`, `fetch/` were collapsed into `workflows/`, `services/`, and `providers/`
...
Tasks:
1. Introduce central contracts and module registry / wiring
2. Make `pytest` and dev dependencies actually runnable in environment
...
5. legacy compatibility layers are fully removed
