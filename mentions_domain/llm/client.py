...
from mentions_domain.llm.pricing import cost_usd
from mentions_domain.llm.retry import CircuitBreaker, CircuitOpenError, with_retry
...
class AnthropicClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._breaker = CircuitBreaker(threshold=breaker_threshold, cooldown_seconds=breaker_cooldown)
    ...
    def _client(self):
        ...
        try:
            ...
            raise RuntimeError(
                'anthropic package not installed. Run `pip install anthropic` to enable AnthropicClient.'
            ) from exc
        if not self._api_key:
            raise RuntimeError('ANTHROPIC_API_KEY not set. Set it in the environment before using AnthropicClient.')
    ...
    def complete(
        ...
        *,
        ...
        def _on_retry(attempt, exc, delay):
            ...
            trace_event('llm.retry', model=model, attempt=attempt, error=str(exc), delay=delay)
        ...
        try:
            ...
            trace_event('llm.call', model=model, ok=False, circuit_open=True, error=str(exc))
        ...
        metrics.incr('llm.cost_micro_usd', n=int(round(call_cost * 1_000_000)), tags={'model': model})
