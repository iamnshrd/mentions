...
**How.** New module `library/_core/eval/posterior_by_horizon.py`. Join
...
**How.** New module `library/_core/retrieve/rerank.py` with a pluggable
...
**What.** Introduce `library/api.py` as the single import surface for
external callers (CLI, future HTTP, notebooks). Re-exports the
curated set of functions with full type hints. Everything under
`library/_core/` becomes officially internal (enforced via
`__all__` and a lint rule).

**Why.** Right now callers reach into `library._core.retrieve.hybrid`
directly. When we refactor hybrid.py internals, we break every
caller. A stable façade lets internals move freely.
...
**How.** New `library/errors.py` with the hierarchy. Migrate one
...
**What.** Settings are currently scattered: `library/config.py`
holds paths; magic numbers (half-life 365d, MMR λ=0.5, RRF k=60)
live at call sites. Consolidate into `library/settings.py` (dataclass
or Pydantic) with env overrides.
...
**What.** `library/_core/ops/integrity.py` — weekly-cron-friendly
check that asserts: every `transcript_chunks.document_id` resolves;
...
on `library/api.py` boundary, gradual elsewhere). Pre-commit hook
runs both on staged files.

**Why.** Catches the 80% of bugs type-hints can catch. Consistent
formatting ends bikeshedding.

**How.** Start with `mypy --strict` only on `library/api.py` and
`library/errors.py` (the boundary). Relax `--ignore-missing-imports`
for `library/_core/*` and tighten one module per PR.
...
`/docs/reference/` from `library/api.py` docstrings. Commit built
