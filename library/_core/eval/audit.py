...
def _check_db() -> dict:
    try:
        ...
        from library._core.kb.migrate import get_schema_version, LATEST_VERSION
...
def _check_config_files() -> dict:
    from library.config import THRESHOLDS, MARKET_CATEGORIES, ANALYSIS_MODES
    missing = []
...
def _check_orchestrator() -> dict:
    try:
        from library._core.runtime.orchestrator import detect_mode, should_use_kb
