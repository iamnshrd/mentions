"""StateStore protocol and canonical key constants for the Mentions agent.

Canonical location: ``library/_core/state_store.py``
(Matches Jordan base-layer pattern — ``library._core.state_store``.)

Modules should import keys from here rather than using raw strings.
``library/state_store.py`` is kept as a compatibility shim that re-exports
